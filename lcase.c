...
lcase_sse_simple(char *restrict dst, const char *restrict src, size_t len)
{
		...
		__m128i case_bit_vector = _mm_set1_epi8(CASE_BIT);

		while (src < src_end)
		{
		    __m128i src_data = LOAD_SI128((const __m128i_u *) src);
		    __m128i lowercased_data = _mm_or_si128(src_data, case_bit_vector);

		    STORE_SI128((__m128i_u *) dst, lowercased_data);
...
lcase_sse_cond(char *restrict dst, const char *restrict src, size_t len)
{
    	...
    	while (src < src_end) {
			__m128i src_data = LOAD_SI128((const __m128i_u *) src);
			__m128i greater_than_a = _mm_cmpgt_epi8(src_data, upper_a);
			__m128i less_than_z = _mm_cmpgt_epi8(upper_z, src_data);
			__m128i in_range = _mm_and_si128(greater_than_a, less_than_z);
			__m128i mask = _mm_and_si128(case_bit, in_range);
			__m128i lowercased_data = _mm_or_si128(src_data, mask);
